...
위 코드는 1999년 발매된 게임 *퀘이크 $3$ 아레나* 의 소스코드 중 일부로, 유명 프로그래머인 존 카멕이 작성했다고 알려져 있으며, 주석에는 이해할 수 없는 코드에 대한 동료 프로그래머의 당혹감이 담겨 있다. 위 함수가 어떻게 $root(1/2, "number")$ 을 반환하는지 지금부터 알아보자.
...
+ ```C i = 0x5f3759df - ( i >> 1 );```
  ...
  이다. 이제 $x = root(1/2, y)$ 로 놓고, $x$ 를 이루는 비트들을 위와 같이 $s, c, d$ 로 나눈 뒤 $j$ 를 $x$ 를 정수로 인식시킨 값이라 하자. 그러면
...
+ ```C y = y * (threehalfs - (x2 * y * y));```
  ...
  주어진 $y$ 는 이제 $root(1/2, "number")$ 에 매우 가까운 값이지만, 여전히 오차가 존재한다. 이 줄에서는 오차를 최소화하기 위해 $y$ 에 뉴턴법을 $1$ 회 적용한다. 뉴턴법을 적용하기 위한 과정은 다음과 같다.
  
  상수 $C$ 에 대해 $x = root(1/2, C)$ 이면 방정식
  ...
  $
    root(1/2, "number") tilde.eq y ( 3 / 2 - (x_2 y^2) / 2 )
  $

  가 된다. 즉, 우리는 $root(1/2, "number")$ 의 근사값을 계산했다.
+ ```C return y;```

   마지막으로 $y$ 의 값을 반환한다. 우리는 위에서 $y tilde.eq root(1/2, "number")$ 임을 보였다.
